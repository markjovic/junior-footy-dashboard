...
// These teams have dropped to C — ALL matches involving them are ignored
const RELEGATED = new Set(['East Ringwood', 'Waverley Blues']);
...
const allMatches = data.matches.filter(m =>
  ...
  !m.isBye && !m.isPartial &&
  !RELEGATED.has(m.home) && !RELEGATED.has(m.away)
);
...
for (const m of allMatches) {
  const hasScore = m.hScore !== undefined && m.aScore !== undefined;
  const isCompleted = hasScore && (m.hScore > 0 || m.aScore > 0);
...
const ladder = Object.values(teams).sort((a, b) => {
  const aMR = a.w + a.d + a.l ? (a.w + a.d * 0.5) / (a.w + a.d + a.l) : 0;
  const bMR = b.w + b.d + b.l ? (b.w + b.d * 0.5) / (b.w + b.d + b.l) : 0;
  const aPCT = a.a ? a.f / a.a : 0;
  const bPCT = b.a ? b.f / b.a : 0;
  return b.pts - a.pts || bMR - aMR || bPCT - aPCT;
}).map((t, i) => ({
  ...t, pos: i + 1,
  mr:  t.w + t.d + t.l ? ((t.w + t.d * 0.5) / (t.w + t.d + t.l) * 100).toFixed(1) : '0.0',
  pct: t.a ? (t.f / t.a * 100).toFixed(1) : '0.0'
}));
...
console.log('\n=== LADDER ===');
console.log('Pos | Team                          | P  | W  | D  | L  | F    | A    | PTS | MR%   | PCT%');
ladder.forEach(t => {
  console.log(
    `${String(t.pos).padStart(3)} | ${t.name.padEnd(30)} | ${String(t.p).padStart(2)} | ${String(t.w).padStart(2)} | ${String(t.d).padStart(2)} | ${String(t.l).padStart(2)} | ${String(t.f).padStart(4)} | ${String(t.a).padStart(4)} | ${String(t.pts).padStart(3)} | ${String(t.mr+'%').padStart(6)} | ${t.pct}%`
  );
});
