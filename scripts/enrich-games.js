...
const VERSION = 'enrich-games v19 2026-09-11 cursor-tdz-fix';
...
async function main() {
  ...
  for (const gradeId of grades) {
    ...
    if (stopped) break;
    // Walked to completion — recorded so a later run skips it. The `break` above
    // has already returned when the budget stopped us mid-grade, so reaching here
    // means the grade finished. v18 added it twice, guarded the second with
    // `if (!stopped)` and left the first bare; both were unreachable when stopped,
    // so the guard read as protection that was not doing anything.
    walked.add(gradeId);
  }
